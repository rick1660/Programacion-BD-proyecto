...
insert into Factura values(4,2019-05-01,30548,3,4,10,223)
insert into Factura values(5,2019-06-24,12054,2,4,10,223)
insert into Factura values(6,2020-12-04,3500,5,2,10,123)
...
SELECT ROUTINE_NAME FROM INFORMATION_SCHEMA.ROUTINES 
   ...
   -- Se agrego campo IdCliente
   alter table Servicios
   add IdCliente int
   --Se agrego llave foranea
   alter table Servicios 
   add foreign key (IdCliente) references Cliente(IdCliente)

   -- Se agrego campos a Refaccion
   Select * from Refaccion
   ...
   where IdRefaccion = 9
      ...
      update Refaccion
   set PiezaSalida = 5
   where IdRefaccion = 10

      update Refaccion
   set PiezaSalida = 1
   where IdRefaccion = 11
   
        update Refaccion
   set PiezaSalida = 37
   where IdRefaccion = 12
 ...
 -- 1
 select Cliente.IdCliente, Cliente.Nombre, Cliente.Apellidos, Servicios.IdServicio , Servicios.Nombre, Servicios.veces_count AS Frecuencia, Servicios.Fecha
 from Cliente INNER JOIN Servicios on Cliente.IdCliente = Servicios.IdCliente  where Servicios.Fecha BETWEEN '1905-06-05' AND '1905-07-02'
 --ORDER BY Cliente.IdCliente

 update Servicios
 set IdDepartamento = 6
 where IdServicio=5
...
select * from Departamentos
-- Agregue Departamento diagnostico
insert into Departamentos values (6, 'Diagnostico', 6149)

select  Departamentos.IdDepartamento, SUM(Servicios.Costo) AS Ventas_Obtenidas from Servicios INNER JOIN Departamentos ON Departamentos.IdDepartamento = Servicios.IdDepartamento where Servicios.IdDepartamento=6
group by Departamentos.IdDepartamento

-- 3
select * from Factura
select Empleado.IdEmpleado, SUM(Factura.Total) AS Venta_Obtenida_Por_Servicio from Factura INNER JOIN Empleado ON Empleado.IdEmpleado=Factura.IdEmpleado 
group by Empleado.IdEmpleado
Order By Venta_Obtenida_Por_Servicio desc

-- 4
select * from Servicios
Select Servicios.IdDepartamento, SUM(veces_count) AS Mas_Concurrentes from Servicios INNER JOIN Departamentos ON Departamentos.IdDepartamento = Servicios.IdDepartamento
group by Servicios.IdDepartamento 
--Order By  Servicios.IdDepartamento  ASC
...
Select Servicios.IdDepartamento, SUM(veces_count) AS Mas_Concurrentes from Servicios INNER JOIN Departamentos ON Departamentos.IdDepartamento = Servicios.IdDepartamento
where Servicios.Fecha BETWEEN '1894-06-05' AND '1905-07-02'
group by Servicios.IdDepartamento 
...
Select Refaccion.IdDepartamento, SUM(Refaccion.Costo * Refaccion.PiezaSalida) AS Venta_Obtenida_por_Refaccion from Departamentos INNER JOIN Refaccion on Departamentos.IdDepartamento = Refaccion.IdDepartamento
group by Refaccion.IdDepartamento

-- 7
select IdVehiculo, NumSerie as Numero_de_Serie, Marca, Modelo, Color, Año, Feche_Registro as Fecha_de_Registro_del_Automovil, IdCliente  from Automovil
where Feche_Registro BETWEEN '1905-06-24' AND '2012-12-31'
...
select Factura.IdCliente, SUM(Factura.Total) AS Gasto_Total_en_Servicios from Cliente INNER JOIN Factura on Cliente.IdCliente = Factura.IdCliente
Group By Factura.IdCliente
Order By SUM(Factura.Total) DESC
...
select  Refaccion.IdRefaccion ,MAX(Costo * Refaccion.PiezaSalida) as Venta_Obtenida from Refaccion JOIN Departamentos on Refaccion.IdDepartamento = Departamentos.IdDepartamento where Refaccion.IdDepartamento = 5
Group by  Refaccion.IdRefaccion
--Order by Refaccion.Costo ASC



select  MAX(Costo * Refaccion.PiezaSalida) as Venta_Obtenida from Refaccion JOIN Departamentos on Refaccion.IdDepartamento = Departamentos.IdDepartamento where Refaccion.IdDepartamento = 5
...
-- Agregue llave foranea Empleado a Servicios
alter table Servicios
add IdEmpleado int
